...
Le second type d'apprentissage est dit supervisé. Dans ce cas, le but est d'apprendre du modèle pour arriver à déterminer la classe de l'observation. Ce modèle d'apprentissage est obtenu grâce à des données dont les classes sont connues. Il existe différentes applications de l'apprentissage supervisé, telles que les arbres de décision, les k plus proches voisins et les réseaux de neurones. Nous nous intéresserons uniquement aux arbres de décision et à la méthode des k plus proches voisins. Les jeux de données sont alors séparés en deux : une partie jeu d'entraînement (\textit{training set}) et une partie jeu de test (\textit{test set}). Le but est d'apprendre du jeu d'entraînement afin de classer les observations du jeu de test.\\
